...
export function ArtworkDetail() {
  ...
  async function makeStorybook() {
    ...
    setCreating(true); setError("");
    try {
      const response = await studentFetch("/api/storybooks", { method: "POST", body: JSON.stringify({ artworkId: artwork.id, format: "landscape" }) });
    ...
    } catch (cause) { setError(cause instanceof Error ? cause.message : "그림책을 만들지 못했어요."); setCreating(false); }
  }
  ...
  return <main className="app-shell artwork-detail-page"><header className="app-header"><Logo /><a className="small-button" href="/student/archive">← 내 그림</a></header>
    ...
    {artwork && <article className="artwork-detail-card"><div className="artwork-detail-copy"><p className="eyebrow">🌟 완성한 작품</p><h1>{artwork.title}</h1><p>{artwork.intent}</p><p className="artwork-readonly-note">🔒 읽기 전용 · 완성한 그림은 그대로 안전하게 보관돼요.</p><time dateTime={artwork.completedAt ?? artwork.updatedAt}>{new Date(artwork.completedAt ?? artwork.updatedAt).toLocaleDateString("ko-KR")}</time></div>
      ...
      <div className="artwork-detail-actions"><a className="button secondary" href="/student/archive">다른 그림 보기</a><button type="button" className="button secondary" disabled={creating} onClick={drawAgain}>🎨 새 그림으로 다시 그리기</button><button type="button" className="button primary" disabled={creating} onClick={makeStorybook}>{creating ? "그림책 준비 중…" : "📖 이 그림으로 그림책 만들기"}</button></div></article>}
  </main>;
}
