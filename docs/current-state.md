...
## 2026-09-09 씨앗 선 12회를 이야기 아크로 구현 (`claude/curriculum-seed-plan-20260909`, 로컬 검증)

- 대상 학년을 초등 1~5학년으로 확정했다(사용자, P-006 종결). 안내 문장은 40자 이내 질문형으로 테스트가 고정한다.
- `lib/arc-content.ts`에 아크 `seed-lines`(제목 `이 선으로 뭘 만들까?`, 12회, version 1, checksum `b037a9d7`)를 추가했다. 회차 형상에 선택 항목 `seed`·`prompts`·`discussion`을 더했고, 체크섬 재료에는 있을 때만 넣어 자전거 아크의 체크섬은 그대로다. 원·반원·나선은 도우미가 폴리라인으로 만들며 기본 도화지(1024×640)에서 둥글게 보이도록 세로 반지름을 1.6배로 잡는다.
- `app/api/artworks/route.ts`: 회차 작품을 처음 만들 때 씨앗을 `pencil` 8px 획으로 문서에 심는다(`seededDocument`). 점선 힌트가 아니라 보통의 획이라 아이가 지울 수 있고 저장 이미지에 남는다. 재사용(두 태블릿 경합) 경로는 그대로다.
- 학생 홈: 오늘 회차 카드가 안내 문장(`prompts`)을 번호 목록으로 보여 주고 음성이 장면 문장과 함께 읽는다. 교사 조종석: 현재 회차에 토론 질문이 있으면 `교실 토론 질문 · 40분 배분` 접이식이 열린다. 학생 응답에는 토론 질문을 싣지 않는다(테스트 고정).
- 검증: `tests/arc-content.test.mjs`에 씨앗 회차 불변식(12회, 11회만 씨앗 없음, 씨앗이 문서 검증 통과, 토론 질문 3개, 문장 길이) 추가. typecheck·lint(경고는 기존 것)·`git diff --check` 통과, 전체 테스트 **320/320**(Node 22.23, `.next` 재빌드 후 — 옛 빌드 산출물이 남아 있으면 typecheck가 삭제된 레슨 라우트 타입을 찾다 실패하므로 `npm test`로 다시 빌드한다). `npm run build && npx next start -p 3299`에 `check:browser` 전 항목 통과. 실제 Chrome(CDP) 실측: 로컬 교사로 학급을 만들고 `seed-lines/seed-line-animal`을 연 뒤 1번 학생으로 입장해 ① 학생 홈 카드에 제목·장면 문장·안내 3문장 표시 ② `그리기 시작` → 새 작품 문서에 `pencil` 획 1개(`x .5, y .25`부터)가 들어가고 1024×640 도화지 가운데 세로선이 그려짐 ③ 교사 학급 화면 조종석에 `교실 토론 질문 · 40분 배분` 접이식과 질문 3개 표시를 확인했다.
- 남은 것: 반 작품 크게 보기 화면, 11회 씨앗 선택 화면, 장면 삽화(10회만 달팽이 사진 재사용). `claude/seed-line-lesson` 브랜치는 이 구현으로 대체되어 폐기 대상이다.
