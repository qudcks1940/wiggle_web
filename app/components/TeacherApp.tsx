...
export function TeacherApp({ classroomId = "" }: { classroomId?: string }) {
  ...
  async function createClass(event: FormEvent) {
    ...
    try {
      // 명단은 선택이다. 비워 두면 예전처럼 아이가 직접 프로필을 만드는 학급이 된다.
      const data = await teacherPost<{ classroom: Classroom }>({ action: "createClassroom", displayName: newClass, ...(parsed.entries.length ? { roster: parsed.entries } : {}) });
