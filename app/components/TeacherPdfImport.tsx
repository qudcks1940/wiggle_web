...
export function TeacherPdfImport({ classroomId, onImported }: { classroomId: string; onImported: () => Promise<unknown> }) {
  ...
  return <section className="book-panel pdf-import-panel">
    ...
    <p className="book-muted">파일당 30MB·1~24쪽. 모든 쪽을 비율 유지·여백 포함 이미지로 가져옵니다. 원본 PDF 글자는 개별 편집되지 않습니다. 가져온 뒤 이야기·그림을 추가하거나 배경을 교체하고, 쪽을 추가·삭제·재배열할 수 있어요. 원본 PDF 파일은 별도로 보관해 주세요.</p>
    ...
    {!!drafts.length && <details><summary>편집 중인 책 · {drafts.length}권</summary><p>수정 또는 중단된 가져오기 내용입니다. 편집기에서 확인 후 완성하기를 눌러 주세요.</p>{drafts.map((d) => <p key={d.id}>{d.seatNumber}번 {d.realName} · {d.title} <a className="small-button" href={`/teacher/class/${classroomId}/books/${d.id}/edit`}>이어서 수정</a></p>)}</details>}
  </section>;
}
