...
export function TeacherBookOrders({ classroomId }: { classroomId: string }) {
  ...
  const [specUid, setSpecUid] = useState("SQUAREBOOK_HC"), [shipping, setShipping] = useState(emptyShipping);
  ...
  return <main className="book-desk">
    ...
    <fieldset disabled={busy} className="print-form-fieldset"><div className="book-order-layout"><div>
      <section className="book-panel"><p className="eyebrow">01 · 원고 준비</p><h2>제작할 책을 선택해요</h2><label className="book-field">제작 판형<select disabled={!state?.specs.length} value={specUid} onChange={(e) => { setSpecUid(e.target.value); setLayout(null); changed(); }}>{!state?.specs.length && <option value="SQUAREBOOK_HC">제작 서비스 연결 대기</option>}{state?.specs.map((s) => <option key={s.bookSpecUid} value={s.bookSpecUid}>{s.name} · {s.bindingType} · {s.pageMin}~{s.pageMax}쪽</option>)}</select></label>
