...
- 몽그리는 아이가 호출했을 때만 개입하고, 모르면 추측하지 않고 한 번에 하나만 질문한다.
- 점선과 시범은 아이 원본 그림과 분리된 레이어이며 저장 이미지에 합성하지 않는다.
- 초등학교 3학년 이상이 핵심 사용자다. 읽기 수준 차이를 고려해 핵심 행동은 그림과 큰 터치 목표로도 이해할 수 있어야 한다(브라우저 읽어 주기 버튼은 2026-09-09 사용자 지시로 제거).
