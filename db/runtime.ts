...
const schemaStatements = [
  ...
  `CREATE TABLE IF NOT EXISTS student_profiles (id TEXT PRIMARY KEY NOT NULL, classroom_id TEXT NOT NULL REFERENCES classrooms(id), nickname TEXT NOT NULL, animal TEXT NOT NULL, class_number INTEGER, last_activity_at TEXT NOT NULL, archived_at TEXT, created_at TEXT NOT NULL DEFAULT CURRENT_TIMESTAMP)`,
...
export async function provisionSchema(DB: D1Database) {
  ...
  if (!studentColumns.results.some((column) => column.name === "class_number")) await DB.prepare(`ALTER TABLE student_profiles ADD COLUMN class_number INTEGER`).run();
