...
<table>
  <tr>
    <td rowspan="2" width="45%" valign="top">
      <img src="./docs/images/entry-tablet.png" alt="학생 입장 — 수첩 숫자판에 참여 코드 여섯 자리를 누르는 화면" />
    </td>
    <td width="55%">
  ...
  <tr>
    <td align="center"><sub>학생 입장 — 이름 대신 참여 코드 한 줄</sub></td>
...
- 🔐 **학생 개인정보를 최소화합니다.** 학생 이메일·학교명을 받지 않습니다. 실명은 교사가 만든 학급 명단에만 있고 그 학급 담임 교사에게만 보이며, 학생 화면·가족 공유·AI 요청에는 실리지 않습니다.
...
| 사용자 | 할 수 있는 일 |
|---|---|
| 🧒 학생 | 첫 화면의 4자리 수업 코드 또는 QR로 반 확인, 선생님이 준 참여 코드 6자리로 입장(처음엔 동물 하나 선택), 단계별 학습, 자유 창작, 몽그리 호출, 그림책 만들기, 그림 소감 남기기 |
...
- 교사가 명단을 만들면 자리마다 붙는 참여 코드 6자리 — 아이는 숫자판으로 코드만 누른다
- 처음 들어올 때만 동물 10종 중 하나를 고르고, 별명은 자동으로 붙는다
- 빈 학급은 신규 생성 화면 자동 진입, 기존 학급은 `새로 시작하기` / `내 그림 이어가기` 분기
- 어느 태블릿에서나 같은 참여 코드로 같은 익명 학생 ID 재입장
...
AI가 책을 대신 만들어 주는 기능이 아니라, 아이가 직접 쓴 이야기와 직접 그린 그림을 한 권의 책으로 엮는 편집 도구입니다. 기존 AI 몽그리가 생각을 끌어내는 질문을 돕고, 편집기에서는 아이가 최종 배치와 표현을 결정합니다. 외부 Canva 화면으로 이동하지 않고 Wiggle 안에서 처음부터 끝까지 작업합니다.
...
- 대문은 전시관 그림과 4자리 코드 입력을 한 화면에 배치하고, 학생 이메일 없이 입장한다는 점을 바로 안내
- 학생 입장 화면은 참여 코드 수첩만 보여 주며 수업 코드를 중복 입력하지 않음
...
- 학생 이메일·출생연도·학교명 미수집 (실명은 교사 학급 명단에만, 담임 교사 전용)
- 원본 device token과 개인 QR token 대신 SHA-256 해시 저장
- 로컬 교사 PIN은 개인 salt를 사용한 PBKDF2-SHA256 100,000회. 학생 참여 코드는 교사 화면에 보여야 하므로 평문 저장(수업 코드와 같은 등급, 교사가 새로 뽑으면 무효)
...
| `/join` | 학급 확인 뒤 참여 코드 6자리 입력, 첫 입장이면 동물 선택 |
| `/join/:token` | QR·토큰 입장 |
