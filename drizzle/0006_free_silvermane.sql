...
CREATE TABLE `storybook_feedback_requests` (
	`id` text PRIMARY KEY NOT NULL,
	`storybook_id` text NOT NULL,
	`classroom_id` text NOT NULL,
	`teacher_id` text NOT NULL,
	`status` text DEFAULT 'waiting_rubric' NOT NULL,
	`rubric_version` text,
	`feedback_json` text,
...
--> statement-breakpoint
CREATE UNIQUE INDEX `storybook_feedback_book_teacher_uq` ON `storybook_feedback_requests` (`storybook_id`,`teacher_id`);--> statement-breakpoint
CREATE INDEX `storybook_feedback_classroom_idx` ON `storybook_feedback_requests` (`classroom_id`,`status`,`requested_at`);--> statement-breakpoint
...
CREATE INDEX `storybooks_classroom_idx` ON `storybooks` (`classroom_id`,`updated_at`);--> statement-breakpoint
ALTER TABLE `artworks` ADD `guide_variant` integer DEFAULT 0 NOT NULL;