...
test("mobile studio and teacher layouts finish in two rows without horizontal text overflow", async () => {
  ...
  assert.match(css, /\.canvas-message,\.save-conflict,\.teacher-viewing \{[^}]*max-width:calc\(100vw - max\(12px,env\(safe-area-inset-left\)\) - max\(12px,env\(safe-area-inset-right\)\)\);[^}]*overflow-wrap:break-word;/);
  ...
  assert.doesNotMatch(studio, /className="tool-panel"|mobile-tool-peek|toolPanelInView/);
  assert.doesNotMatch(css, /\.tool-panel|\.tool-rail|\.tool-colors|mobile-tool-peek/);
